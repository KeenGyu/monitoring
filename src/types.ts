...
export type ActivityType = "created" | "status-changed" | "submitted" | "proof-added" | "proof-removed" | "edited" | "deleted";
export interface ActivityEvent { id: string; timestamp: string; reportId: string; reportName: string; type: ActivityType; message: string; detail?: string; }
export interface MonitoringPeriod { startMonth: string; endMonth: string; }
export interface Profile { name: string; role: string; }
export interface AppSettings { monitoringPeriod: MonitoringPeriod; profile: Profile; }
export interface ExportBundle { version: 1; exportedAt: string; reports: Report[]; activity: ActivityEvent[]; settings: AppSettings; absentees: Absentee[]; }

export type AbsenceType = "Sick Leave" | "Vacation Leave" | "Emergency Leave" | "Late" | "Half-day" | "Undertime" | "AWOL" | "Other";
...
export type NewAbsenteeInput = Pick<Absentee, "employeeName" | "department" | "date" | "type" | "remarks" | "status" | "minutes" | "deductFromSalary">;
export type NewReportInput = Pick<Report, "name" | "category" | "description" | "dueDate" | "monitoringStart" | "monitoringEnd" | "supervisor" | "proofRequired"> & { funnyReactionImage?: string | null };
export interface SubmissionInput { submittedAt: string; supervisor: string; submissionNotes: string; proofImage: string | null; }
